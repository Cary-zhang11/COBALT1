...
**改动 1：删除「五、冒烟测试清单」章节。** 该章节原为全部用例列表，已被新的独立冒烟文档替代。

**改动 2：文件清单表格新增 3 行冒烟分支产出：**
...
**改动 3：文档结构中「五、冒烟测试清单」→「五、开发冒烟用例（独立文档）」**，内容改为指向独立文件的引用说明：

```markdown
## 五、开发冒烟用例

> 开发冒烟用例已作为独立文档产出，详见同目录下：
> - `{需求名称}_三方分析.md` — 三方专家分析报告
> - `{需求名称}_开发冒烟用例.md` — 精简开发冒烟用例（5-20条，全部P0）
> - `{需求名称}_开发冒烟用例.xlsx` — Excel 格式

原「冒烟测试清单」章节（全量用例列表）已移除，其功能由独立冒烟文档替代。
```
